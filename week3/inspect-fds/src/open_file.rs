...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let flink = format!("/proc/{}/fd/{}", pid, fd);
        let fpath = OpenFile::path_to_name(fs::read_link(flink).ok()?.to_str().unwrap());
        let fname = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fdinfo = fs::read_to_string(fname).ok()?;
        let mode = OpenFile::parse_access_mode(&fdinfo).unwrap();
        let cursor = OpenFile::parse_cursor(&fdinfo).unwrap();
        Some(OpenFile::new(fpath, cursor, mode))
    }
